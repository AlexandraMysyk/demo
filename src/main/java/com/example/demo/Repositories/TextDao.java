...
public interface TextDao extends JpaRepository<Text, Integer> {
    ...
    @Query(value = "UPDATE text SET content=? , name=?,complexity_level=? where id_text =?", nativeQuery = true)
    Text update(String content, String name, String complexityLevel, int idText);
